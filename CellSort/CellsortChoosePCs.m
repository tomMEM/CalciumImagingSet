function [PCuse] = CellsortChoosePCs(fn, mixedfilters,handles)
...
%
figure;
fprintf('-------------- CellsortChoosePCs %s -------------- \n', date)%tb replace CellsortICA

[pixw,pixh] = size(imread(fn,1));

npcs = 20;  % Number of PCs to display concurrently TB needs to be same in database
currpcs = [1:npcs];
PCf = [];
while isempty(PCf)
