...
%Version 1.4.1 added line 63 figure; %TB
%
...
if plotting
    ...
    hICSegments = findobj('type', 'figure', 'Name','IC and Segements');    
    if  ishandle(hICSegments)
        set(0, 'currentfigure', hICSegments);
        figure(hICSegments);
    else
        figure('Name','IC and Segements','Position',[1 1 scrsz(3)/1 scrsz(4)/1],'NumberTitle','off');
...
for j = 1:nic
    ...
    if plotting && ~isempty(Lbig)
        ...
        colord = lines(k+length(Lbig));
            ...
            subplot(224)% TB to keep centroids overlayed in one plot
            if co == 1
                imagesc(squeeze(ica_segments(jj+k,:,:)));
                co=co+1;%TB TB to keep centroids overlayed in one plot so takes just one image for scale
            end
            hold off %TB TB to keep centroids overlayed in one plot
        ...
        for jj = 1:length(Lbig)
            ...
            %axis image %TB to keep size of image constant
            %xlim([0,pixw]); ylim([0,pixh])
            
            subplot(224)
            %imagesc(squeeze(ica_segments(jj+k,:,:))) %TB, move up 
            hold on
            plot(segcentroid(jj+k,1), segcentroid(jj+k,2),'bo','color',colord(jj+k,:),'linewidth',2)
